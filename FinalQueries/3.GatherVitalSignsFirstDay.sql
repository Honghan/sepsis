...
select rv.hadm_id, rv.subject_id
...
from (
			select subject_id, hadm_id, time, value, valuenum, valueuom 
			  , case
			    ...
			    when itemid in (779,220224) and value is not null and value is not null then 54 --pao2
	...
	group by rv.hadm_id, rv.subject_id
	order by rv.hadm_id, rv.subject_id